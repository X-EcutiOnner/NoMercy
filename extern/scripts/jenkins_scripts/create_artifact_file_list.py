...
def write_filelist_to_txt(file_list):
	...
	print(f"File created: '{filename}' with {len(file_list)} artifacts")

def main(args):
	workspacePath = os.getenv('WORKSPACE')
	if not workspacePath and not args:
		print("Workspace not found")    
		return 1
	elif not workspacePath and args:
		workspacePath = args[0]
  
	if not os.path.exists(workspacePath):
		print(f"Workspace path: {workspacePath} does not exist")    
		return 1		

	files_found = find_files_in_folders(workspacePath, folders_patterns)
	if files_found:
