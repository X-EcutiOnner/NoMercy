...
message(STATUS "INF2CAT_PATH: ${INF2CAT_PATH}")
...
function(wdk_add_driver _target)
    ...
    set_target_properties(${_target} PROPERTIES VERSION "${WDK_NOMERCY_VERSION}")
    ...
    set(PROJECT_INF_PATH "${PROJECT_SOURCE_DIR}/${WDK_NOMERCY_FILE}.inf")
    set(PROJECT_PFX_PATH "${PROJECT_SOURCE_DIR}/${WDK_PFX_FILE}")
    ...
    if(HOST_ARCH_X86)
        set(STAMPINF_ARGS "-k \"1.9\" -d \"*\" -a \"x86\" -v \"${WDK_NOMERCY_VERSION}\" -f")
    elseif(HOST_ARCH_X64)
        set(STAMPINF_ARGS "-k \"1.9\" -d \"*\" -a \"amd64\" -v \"${WDK_NOMERCY_VERSION}\" -f")
    elseif(HOST_ARCH_ARM64)
        set(STAMPINF_ARGS "-k \"1.9\" -d \"*\" -a \"arm64\" -v \"${WDK_NOMERCY_VERSION}\" -f")
    endif()
    ...
    add_custom_command(
      ...
      TARGET ${_target}
      POST_BUILD
      COMMAND ${STAMPINF_PATH} ${STAMPINF_ARGS} "${WDK_NOMERCY_FILE}.inf"
      WORKING_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}"
    )

    # sign the files if certificate is available
    if(WDK_PFX_FILE AND EXISTS ${PROJECT_PFX_PATH})
        ...
        add_custom_command(
          ...
          TARGET ${_target}
          POST_BUILD
          COMMAND ${SIGNTOOL_PATH} ${SIGNTOOL_ARGS} "${CMAKE_CURRENT_BINARY_DIR}/${WDK_NOMERCY_FILE}.SYS"
        )
    endif()
endfunction()
