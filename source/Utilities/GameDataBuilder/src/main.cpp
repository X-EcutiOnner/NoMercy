...
#define LOG_FILENAME "nmgd_generator.log"

int32_t main(int32_t argc, char* argv[])
{
	std::string in_filename = "../document/internal/NoMercy_game_data.json";
	if (argc == 2)
		in_filename = argv[1];

	LogfA(LOG_FILENAME, "Input file: %s\n", in_filename.c_str());

	// Open files
	msl::file_ptr in_file(in_filename, "rb");
	if (!in_file)
	{
		LogfA(LOG_FILENAME, "%s could not open! Error: %u", in_filename.c_str(), errno);
		return EXIT_FAILURE;
	}

	msl::file_ptr out_file("NoMercy_Game.dat", "wb");
	if (!out_file)
	{
		LogfA(LOG_FILENAME, "NoMercy_Game.dat could not open! Error: %u", errno);
		return EXIT_FAILURE;
	}
	...
	LogfA(LOG_FILENAME, "Input file size: %u Hash: %u\n", in_size, XXH32(in_data.data(), in_size, 0));
	...
	LogfA(LOG_FILENAME, "Compression success! Size: %u/%u Bound: %u Compressed: %u\n",
		compressedsize, compressed.size(), bound, XXH32(compressed.data(), compressedsize, 0)
	);
	...
	LogfA(LOG_FILENAME, "Encryption success! Size: %u Encrypted: %u\n", crypted.size(), XXH32(crypted.data(), crypted.size(), 0));

	/// Write basic data
	// Magic
	const uint32_t magic = NM_CREATEMAGIC('N', 'M', 'G', 'D');
	out_file.write(&magic, sizeof(magic));

	// Version
	const uint32_t version = NOMERCY_GAME_DATA_VERSION;
