...
// TODO: replace with https://github.com/cppfastio/fast_io
...
public:
	...
	size_t write_ex(const std::vector<char>& vec) const { return std::fwrite(vec.data(), vec.size(), 1, m_ptr_); }
	...
	size_t string_write_ex(const std::string& str) const { return std::fwrite(str.data(), str.size(), 1, m_ptr_); }
