...
//
static UINT WriteIconHeader(HANDLE hFile, WORD nImages)
{
...
static BOOL GetIconBitmapInfo(HICON hIcon, ICONINFO *pIconInfo, BITMAP *pbmpColor, BITMAP *pbmpMask)
{
	...
	if (!GetObjectA(pIconInfo->hbmColor, sizeof(BITMAP), pbmpColor))
		return FALSE;

	if (!GetObjectA(pIconInfo->hbmMask, sizeof(BITMAP), pbmpMask))
		return FALSE;
...
//
static BOOL SaveIcon(const wchar_t *wszIconFile, HICON hIcon[], WORD nNumIcons)
{
	HANDLE hFile;
	int i;
	int * pImageOffset;

	if (hIcon == 0 || !nNumIcons)
		return FALSE;

	// Save icon to disk:
	hFile = CreateFileW(wszIconFile, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);
