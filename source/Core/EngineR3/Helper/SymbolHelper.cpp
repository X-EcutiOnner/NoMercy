...
namespace Scoped
{
	...
	public:
		...
		explicit Inet(const HINTERNET hInet) :
			m_hInet(hInet)
		{
		}
		~Inet()
		...
		void close() noexcept
		{
			if (m_hInet)
			{
				g_winAPIs->InternetCloseHandle(std::exchange(m_hInet, nullptr));
			}
		}
	private:
		HINTERNET m_hInet;
...
namespace Pdb
{
	...
	{
		const Scoped::Inet hInet(g_winAPIs->InternetOpenW(xorstr_(L"HttpDownloader"), INTERNET_OPEN_TYPE_DIRECT, nullptr, nullptr, 0));
		if (!hInet)
		{
			return false;
		}

		const Scoped::Inet hUrl(g_winAPIs->InternetOpenUrlW(hInet, url, nullptr, 0, INTERNET_FLAG_SECURE | INTERNET_FLAG_NO_COOKIES | INTERNET_FLAG_NO_CACHE_WRITE | INTERNET_FLAG_RESYNCHRONIZE, 0));
		if (!hUrl)
		{
			return false;
		}
		...
		const auto queryHttpDword = [](const HINTERNET hUrl, const unsigned long info) -> std::pair<unsigned long, bool> {
			...
			const bool status = !!g_winAPIs->HttpQueryInfoW(hUrl, info | HTTP_QUERY_FLAG_NUMBER, &result, &sizeOfResult, &index);
		...
		while (g_winAPIs->InternetReadFile(hUrl, bufPtr, static_cast<unsigned long>(buf.size()), &readBytes) && readBytes)
		{
	...
	HANDLE WinInetFileDownloader::createFileWithHierarchy(const wchar_t* filePath, const unsigned long access, const unsigned long share) noexcept
	{
		...
		const auto discardChanges = [](const std::vector<size_t> dirs, wchar_t* const mutablePath) {
			for (auto it = dirs.crbegin(); it != dirs.crend(); ++it)
			{
				...
				g_winAPIs->RemoveDirectoryW(mutablePath);
		...
		for (wchar_t& sym : path)
		{
			...
			if (isSlash(sym))
			{
				...
				sym = L'\0';
				const bool status = !!g_winAPIs->CreateDirectoryW(pathBuf, nullptr);
				if (status)
				{
		...
		HANDLE hFile = g_winAPIs->CreateFileW(filePath, access, share, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
	...
	WinInetAbstractDownloader::Action WinInetFileDownloader::onReceive(const void* buf, size_t size)
	{
		...
		const bool status = !!g_winAPIs->WriteFile(m_hFile, buf, static_cast<unsigned int>(size), &written, nullptr);
		if (!status)
		{
	...
	void WinInetFileDownloader::closeFile() noexcept
	{
		if (valid())
		{
			g_winAPIs->CloseHandle(std::exchange(m_hFile, INVALID_HANDLE_VALUE));
		}
	}

	WinInetFileDownloader::WinInetFileDownloader(const wchar_t* filePath) noexcept :
