...
namespace NoMercy
{
	...
	bool CBCDHelper::Initialize()
	{
		...
		const auto ntStatus = g_winAPIs->BcdOpenSystemStore(&m_hBCDStore);
	...
	void CBCDHelper::Release()
	{
		if (m_hBCDStore)
		{
			const auto ntStatus = g_winAPIs->BcdCloseStore(m_hBCDStore);
			if (!NT_SUCCESS(ntStatus))
			{
	...
	{
		if (!g_winAPIs->BcdSetLogging)
			return;
		
		const auto ntStatus = g_winAPIs->BcdSetLogging(BCD_MESSAGE_TYPE_TRACE, [](BCD_MESSAGE_TYPE type, PWSTR Message) {
	...
	bool CBCDHelper::OpenObject(PGUID pvIdentifierGUID, PHANDLE phObject)
	{
		...
		const auto ntStatus = g_winAPIs->BcdOpenObject(m_hBCDStore, pvIdentifierGUID, &hBCDObject);
		if (!NT_SUCCESS(ntStatus))
		{
	...
	bool CBCDHelper::CloseObject(HANDLE hObject)
	{
		...
		const auto ntStatus = g_winAPIs->BcdCloseObject(hObject);
		if (!NT_SUCCESS(ntStatus))
		{
	...
	bool CBCDHelper::GetElementData(HANDLE hObject, ULONG ulElementType, PVOID pvBuffer, PULONG pulBufferSize)
	{
		...
		const auto ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, pvBuffer, pulBufferSize);
		if (!NT_SUCCESS(ntStatus))
		{
	...
	bool CBCDHelper::GetElementDataFrom(ULONG ulElementType, GUID guidID, PVOID pvBuffer, PULONG pulBufferSize)
	{
		...
		HANDLE hBCDObject = nullptr;
		auto ntStatus = g_winAPIs->BcdOpenObject(m_hBCDStore, &guidID, &hBCDObject);
		if (!NT_SUCCESS(ntStatus))
		{
			APP_TRACE_LOG(LL_ERR, L"BCD object open failed with status: %p", ntStatus);
			return false;
		...
		ntStatus = g_winAPIs->BcdGetElementData(hBCDObject, ulElementType, pvBuffer, pulBufferSize);
		if (!NT_SUCCESS(ntStatus))
		{
			APP_TRACE_LOG(LL_ERR, L"BCD element data get failed with status: %p", ntStatus);
		}
		...
		if (hBCDObject)
		{
			g_winAPIs->BcdCloseObject(hBCDObject);
			hBCDObject = nullptr;
		}
	...
	bool CBCDHelper::SetElementData(HANDLE hObject, ULONG ulElementType, PVOID pvBuffer, ULONG BufferSize)
	{
		...
		const auto ntStatus = g_winAPIs->BcdSetElementData(hObject, ulElementType, pvBuffer, BufferSize);
		if (!NT_SUCCESS(ntStatus))
		{
	...
	bool CBCDHelper::GetElementDevice(HANDLE hObject, ULONG ulElementType, std::shared_ptr <BCD_ELEMENT_DEVICE>& spElementData)
	{
		...
		auto spValueBuffer = stdext::make_shared_nothrow<BCD_ELEMENT_DEVICE>();
		ULONG ulValueLength = sizeof(BCD_ELEMENT_DEVICE);
		auto ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, spValueBuffer.get(), &ulValueLength);
		if (!NT_SUCCESS(ntStatus))
		{
			APP_TRACE_LOG(LL_ERR, L"BCD element data get failed with status: %p", ntStatus);
			return false;
	...
	bool CBCDHelper::GetElementString(HANDLE hObject, ULONG ulElementType, std::wstring& wstElementData)
	{
		...
		auto ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, wstBuffer.data(), &ulStringLength);
		if (ntStatus == STATUS_BUFFER_TOO_SMALL)
		{
			wstBuffer.resize(ulStringLength);

			ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, wstBuffer.data(), &ulStringLength);
		}
	...
	bool CBCDHelper::GetElementObject(HANDLE hObject, ULONG ulElementType, GUID& pkElementData)
	{
		...
		const auto ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, &pkValue, &ulValueLength);
		if (!NT_SUCCESS(ntStatus))
		{
			APP_TRACE_LOG(LL_ERR, L"BCD element data get failed with status: %p", ntStatus);
			return false;
	...
	bool CBCDHelper::GetElementObjectList(HANDLE hObject, ULONG ulElementType, std::vector <GUID>& vecElementData)
	{
		...
		auto ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, vecValue.data(), &ulValueLength);
		...
		if (ntStatus == STATUS_BUFFER_TOO_SMALL)
		{
			...
			ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, vecValue.data(), &ulValueLength);
			if (!NT_SUCCESS(ntStatus))
			{
	...
	bool CBCDHelper::GetElementInteger(HANDLE hObject, ULONG ulElementType, ULONG64& pul64ElementData)
	{
		...
		auto ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, &ul64ValueBuffer, &ulValueLength);
		if (!NT_SUCCESS(ntStatus))
		{
			APP_TRACE_LOG(LL_ERR, L"BCD element data get failed with status: %p", ntStatus);
			return false;
	...
	bool CBCDHelper::GetElementBoolean(HANDLE hObject, ULONG ulElementType, bool& pkElementData)
	{
		...
		auto ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, &boValueBuffer, &ulValueLength);
		if (!NT_SUCCESS(ntStatus))
		{
			...
			if (ulValueLength == 2)
			{
				...
				ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, &u16ValueBuffer, &ulValueLength);
				tempBuffer = u16ValueBuffer;
			}
			else if (ulValueLength == 4)
			{
				uint32_t u32ValueBuffer = 0;
				ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, &u32ValueBuffer, &ulValueLength);
				tempBuffer = u32ValueBuffer;
			}
			else if (ulValueLength == 8)
			{
				uint64_t u64ValueBuffer = 0;
				ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, &u64ValueBuffer, &ulValueLength);
	...
	bool CBCDHelper::GetElementBinary(HANDLE hObject, ULONG ulElementType, std::vector <uint8_t>& vElementData)
	{
		...
		auto ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, vBuffer.data(), &ulValueLength);
		if (ntStatus != STATUS_BUFFER_TOO_SMALL && !NT_SUCCESS(ntStatus))
		{
			APP_TRACE_LOG(LL_ERR, L"BCD element data get (1) failed with status: %p", ntStatus);
			return false;
		...
		if (ntStatus == STATUS_BUFFER_TOO_SMALL)
		{
			...
			ntStatus = g_winAPIs->BcdGetElementData(hObject, ulElementType, vBuffer.data(), &ulValueLength);
	...
	NTSTATUS CBCDHelper::EnumerateElements(HANDLE hObject, PVOID pvBuffer, PULONG pulBufferSize, PULONG pulElementCount)
	{
		...
		const auto ntStatus = g_winAPIs->BcdEnumerateAndUnpackElements(m_hBCDStore, hObject, kEnumFlags, pvBuffer, pulBufferSize, pulElementCount);
		if (!NT_SUCCESS(ntStatus))
		{
	...
	NTSTATUS CBCDHelper::EnumerateObjects(PBCD_OBJECT_DESCRIPTION pkEnumDescriptor, PVOID pvBuffer, PULONG pulBufferSize, PULONG pulObjectCount)
	{
		...
		const auto ntStatus = g_winAPIs->BcdEnumerateObjects(m_hBCDStore, pkEnumDescriptor, pvBuffer, pulBufferSize, pulObjectCount);
		if (!NT_SUCCESS(ntStatus))
		{
	...
	bool CBCDHelper::EnumerateOsLoaderList(std::vector <std::shared_ptr <SBCDObjectEntry>>& vecObjects)
	{
		...
		if (ntStatus == STATUS_BUFFER_TOO_SMALL)
		{
			pObjectBuffer = static_cast<PBCD_OBJECT>(CMemHelper::Allocate(ulObjectSize));
		...
		for (std::size_t i = 0; i < ulObjectCount; i++)
		{
			...
			{
				auto entry = stdext::make_shared_nothrow<SBCDObjectEntry>();
				if (IS_VALID_SMART_PTR(entry))
				{
		...
		{
			HANDLE hObject = nullptr;
			std::wstring wstObjectDescription;
			if (this->OpenObject(&GUID_WINDOWS_MEMORY_TESTER, &hObject))
			{
				if (this->GetElementString(hObject, BcdLibraryString_Description, wstObjectDescription))
				{
					auto entry = stdext::make_shared_nothrow<SBCDObjectEntry>();
					if (IS_VALID_SMART_PTR(entry))
					{
						memcpy(&entry->guidObject, &GUID_WINDOWS_MEMORY_TESTER, sizeof(GUID));
						entry->wstObjectName = wstObjectDescription;
	...
	bool CBCDHelper::EnumerateValueObjects(PGUID Identifier, ULONG ElementType, std::vector <std::shared_ptr <SBCDObjectValueEntry>>& vecObjects)
	{
		...
		for (ULONG i = 0; i < ulObjectListLength / sizeof(BCD_ELEMENT_OBJECT_LIST); i++)
		{
			...
			auto entry = stdext::make_shared_nothrow<SBCDObjectValueEntry>();
			if (IS_VALID_SMART_PTR(entry))
			{
	...
	std::vector <std::shared_ptr <SBCDObjectEntry>> CBCDHelper::QueryBootApplicationList(bool EnumerateAllObjects)
	{
		...
		{
			this->EnumerateBootMgrList(&GUID_WINDOWS_BOOTMGR, BcdBootMgrObjectList_DisplayOrder, vecObjects);

			this->EnumerateBootMgrList(&GUID_WINDOWS_BOOTMGR, BcdBootMgrObjectList_ToolsDisplayOrder, vecObjects);
		}
	...
	std::vector <std::shared_ptr <SBCDObjectEntry>> CBCDHelper::QueryFirmwareBootApplicationList()
	{
		...
		this->EnumerateBootMgrList(&GUID_FIRMWARE_BOOTMGR, BcdBootMgrObjectList_DisplayOrder, vecObjects);

		this->EnumerateBootMgrList(&GUID_FIRMWARE_BOOTMGR, BcdBootMgrObjectList_ToolsDisplayOrder, vecObjects);
