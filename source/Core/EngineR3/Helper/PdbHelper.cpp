...
namespace Pdb
{
	const wchar_t* Prov::k_microsoftSymbolServer        = xorstr_(L"http://msdl.microsoft.com/download/symbols");
	const wchar_t* Prov::k_microsoftSymbolServerSecure  = xorstr_(L"https://msdl.microsoft.com/download/symbols");
	const wchar_t* Prov::k_defaultSymPath               = xorstr_(L"srv*C:\\Symbols*http://msdl.microsoft.com/download/symbols");
	...
	InstUid Prov::uid()
	{
		if (!s_initCount)
		{
			throw NotInitialized(xorstr_(L": A symbols provider isn't created yet. Create the Pdb::Prov instance before the call."));
		}
		return &s_initCount;
	}
	...
	PdbInfo PdbInfo::get(const wchar_t* const path) noexcept(false)
	{
		...
		const bool status = !!g_winAPIs->SymSrvGetFileIndexInfoW(path, &info, 0);
		if (!status)
		{
			const auto lastError = g_winAPIs->GetLastError();
			throw DbgHelpFailure(xorstr_(L": Unable to get a file index info."), lastError);
		}
	...
	Prov::Prov(const wchar_t* symPath) noexcept(false)
	{
		if (!s_initCount)
		{
			const bool status = !!g_winAPIs->SymInitializeW(&s_initCount, symPath, false);
			if (!status)
			{
				const auto lastError = g_winAPIs->GetLastError();
				throw DbgHelpFailure(xorstr_(L": Unable to create the Prov instance: 'SymInitializeW' failure."), lastError);
			}
	...
	Prov::~Prov()
	{
		...
		if (!s_initCount)
		{
			g_winAPIs->SymCleanup(&s_initCount);
		}
	}

	uint32_t Prov::getOptions() const noexcept
	{
		return g_winAPIs->SymGetOptions();
	}
	...
	std::wstring Prov::getSymPath() const noexcept(false)
	{
		...
		while (true)
		{
			const bool status = !!g_winAPIs->SymGetSearchPathW(uid(), &buf[0], static_cast<uint32_t>(buf.size()));
			if (status)
			{
	...
	{
		const bool status = !!g_winAPIs->SymSetSearchPathW(uid(), symPath);
		if (!status)
		{
			const auto lastError = g_winAPIs->GetLastError();
	...
	const wchar_t* const TypeHolder<BaseType>::s_names[]
	{
		...
		xorstr_(L"char"),
		xorstr_(L"wchar_t"),
		xorstr_(L"signed char"),
		xorstr_(L"unsigned char"),
		xorstr_(L"int"),
		xorstr_(L"unsigned int"),
		xorstr_(L"float"),
		xorstr_(L"<BCD>"),
		xorstr_(L"bool"),
		xorstr_(L"short"),
		xorstr_(L"unsigned short"),
		xorstr_(L"long"),
		xorstr_(L"unsigned long"),
		xorstr_(L"__int8"),
		xorstr_(L"__int16"),
		xorstr_(L"__int32"),
		xorstr_(L"__int64"),
		xorstr_(L"__int128"),
		xorstr_(L"unsigned __int8"),
		xorstr_(L"unsigned __int16"),
		xorstr_(L"unsigned __int32"),
		xorstr_(L"unsigned __int64"),
		xorstr_(L"unsigned __int128"),
		xorstr_(L"<currency>"),
		xorstr_(L"<date>"),
		xorstr_(L"VARIANT"),
		xorstr_(L"<complex>"),
	...
	const wchar_t* const TypeHolder<DataKind>::s_names[]
	{
		xorstr_(L"Unknown"),
		xorstr_(L"Local"),
		xorstr_(L"Static Local"),
		xorstr_(L"Param"),
		xorstr_(L"Object Ptr"),
		xorstr_(L"File Static"),
		xorstr_(L"Global"),
		xorstr_(L"Member"),
		xorstr_(L"Static Member"),
		xorstr_(L"Constant")
	};
	...
	Sym Children::find(const wchar_t* name) const noexcept(false)
	{
		if (!name)
		{
			throw SymNotFound(__FUNCTIONW__ L": Name is NULL.", xorstr_(L"<null>"));
		}
		...
		throw SymNotFound(std::wstring(xorstr_(L": Symbol '")).append(name).append(xorstr_(L"' not found.")), name);
	}
	...
	{
		return !!g_winAPIs->SymGetTypeInfo(
			Prov::uid(),
			m_mod.base(),
			m_typeId,
	...
	{
		m_base = g_winAPIs->SymLoadModuleExW(Prov::uid(), nullptr, path, synonym, imageBase, imageSize, nullptr, 0);
		if (!m_base)
		{
			const auto lastError = g_winAPIs->GetLastError();
			throw DbgHelpFailure(xorstr_(L": Unable to load module: 'SymLoadModuleExW' failure."), lastError);
		}
	}
	Mod::Mod(Mod&& mod) noexcept :
	...
	Mod::~Mod()
	{
		if (m_base)
		{
			g_winAPIs->SymUnloadModule64(Prov::uid(), m_base);
		}
	}
	...
	Sym Mod::find(const wchar_t* name) const noexcept(false)
	{
		if (!name)
		{
			throw SymNotFound(xorstr_(L": Name is NULL."), xorstr_(L"<null>"));
		}
		...
		const bool status = !!g_winAPIs->SymGetTypeFromNameW(Prov::uid(), base(), name, info);
		if (!status)
		{
			const auto lastError = g_winAPIs->GetLastError();
			...
			switch (lastError)
			{
				...
				{
					throw DbgHelpFailure(xorstr_(L": Unable to get type from name: 'SymGetTypeFromNameW' failure. "), lastError);
				}
				default:
				{
					throw DbgHelpFailure(xorstr_(L": Unable to get type from name: 'SymGetTypeFromNameW' failure."), lastError);
