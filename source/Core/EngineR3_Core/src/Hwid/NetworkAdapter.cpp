...
{
#define ALLOCATE_FROM_PROCESS_HEAP( bytes )		g_winAPIs->HeapAlloc(g_winAPIs->GetProcessHeap(), HEAP_ZERO_MEMORY, bytes)
#define DEALLOCATE_FROM_PROCESS_HEAP( ptr )		if (ptr) g_winAPIs->HeapFree(g_winAPIs->GetProcessHeap(), 0, ptr)
#define REALLOC_FROM_PROCESS_HEAP( ptr, bytes )	g_winAPIs->HeapReAlloc(g_winAPIs->GetProcessHeap(), HEAP_ZERO_MEMORY, ptr, bytes)
	...
	DWORD EnumNetworkAdapters(CNetworkAdapter* pAdapters, ULONG ulSzBuf, LPDWORD lpdwOutSzBuf)
	{
		...
		erradapt = g_winAPIs->GetAdaptersInfo(pAdptInfo, &ulLen);
	...
	bool CNetworkAdapter::SetupAdapterInfo(IP_ADAPTER_INFO* pAdaptInfo)
	{
		...
		if (pAdaptInfo)
		{
			m_sName = stdext::to_wide(pAdaptInfo->AdapterName);
			...
			{
				m_sCurIpAddr.sIp = xorstr_("0.0.0.0");
				m_sCurIpAddr.sSubnet = xorstr_("0.0.0.0");
			}
			...
			err = g_winAPIs->GetPerAdapterInfo(m_dwIndex, pPerAdapt, &ulLen);
	...
	std::string CNetworkAdapter::GetAdapterTypeString(UINT nType)
	{
		...
		case MIB_IF_TYPE_ETHERNET:
			sType = xorstr_("Ethernet");
			break;
		case MIB_IF_TYPE_TOKENRING:
			sType = xorstr_("Token Ring");
			break;
		...
		case MIB_IF_TYPE_LOOPBACK:
			sType = xorstr_("Loopback");
			break;
		case MIB_IF_TYPE_SLIP:
			sType = xorstr_("SLIP");
			break;
		default:
			sType = xorstr_("Invalid Adapter Type");
			break;
		};
	...
	std::wstring CNetworkAdapter::GetAdapterName() const
	{
		return m_sName;
	}
	...
	std::wstring CNetworkAdapter::GetAdapterAddress(void)
	{
		std::string stTemp;
		char macTemp[8]{ '\0' };
		for (unsigned int i = 0; i < m_ucAddress.nLen; i++)
		{
			...
			sprintf_s(macTemp, sizeof(macTemp), xorstr_("%02X"), m_ucAddress.ucAddress[i]);
			stTemp += macTemp;
		}

		return stdext::to_wide(stTemp);
	}
};
