...
#pragma pack(pop)
	...
	enum class smbios_table_types : std::uint8_t
	{
		bios_information = 0,
		system_information = 1,
		baseboard_informations = 2,
	...
	namespace detail
	{
		...
		struct smbios_table_entry_t
		{
			...
			smbios_table_entry_t(table_header_t* const table_header, entry_handle e_handle, const table_string_container& _table_strings) :
				strings(_table_strings), handle(e_handle)
			{
		...
		public:
			...
			smbios_table(const smbios_table_entry_t& entry) :
				m_entry(entry)
			{
			}
