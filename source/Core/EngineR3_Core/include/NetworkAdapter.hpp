...
#define DEFAULT_GATEWAY_ADDR 0
	...
	extern DWORD EnumNetworkAdapters(CNetworkAdapter* lpBuffer, ULONG ulSzBuf, LPDWORD lpdwOutSzBuf);
	...
	private:
		std::wstring			m_sName;		// adapter name with the computer.  For human readable name use m_sDesc.
		std::string			m_sDesc;
	...
	public:
		...
		std::wstring GetAdapterAddress(void);
	};
};
