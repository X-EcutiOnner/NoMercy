...
#define DESCRIPTOR_DATA_OFFSET 4
    ...
    struct SMonitorCtx
    {
        std::string stManufacturer;	// Monitor manufacturer
        std::string stType;			// Monitor type
        std::string stCaption;		// Caption
        std::string stDescription;	// Description
        std::string stSerial;		// Serial
    };
    ...
    public:
        ...
        bool Connect(const std::wstring& stDevice);
        void Disconnect();

        bool IsConnected() const;

        bool GetMonitors(std::vector <SMonitorCtx>& vMonitors);
    ...
    protected:
        ...
        std::string	GetManufacturerID(BYTE ID[2]);
        ...
        std::string	DecodeDPMSFlag(BYTE Flag);

        // Parse TEXT zone of Detailed_Timing
        std::string	GetEdidText(BYTE lpByte[18]);

        // Get Detailed_Timing type
        DetailTiming GetDetailledTimingDescriptionType(BYTE Detailed_Timing_Descript[]);
        std::string	GetDescription(Standard_EDID* myRecord);

        // Acer monitor specific because of bogus serial
        bool		AcerHack(std::string& stSerial, Standard_EDID* myRecord);

    private:
        HDEVINFO    m_hDevInfo;
        std::string m_stDeviceName;
    };
};
